...
The DeliveryFrameController class is a JavaFX controller for a  Delivery Frame in EKRUT Client.

The class uses the FXML framework to define the layout of the Choose Delivery  and to handle events.

@author [Shahar]

@version [4.0]
*/
public class DeliveryFrameController extends FrameController {
	...
	*/
	...
	public void additionalChanges() {
		...
		for (Delivery deliv : arr_delivery_table) {
			if(deliv.getStatus().toString().equals("Active")) {
	...
	*/
	public void getUpdateBtn(ActionEvent event) {
		if(isUpdateButtonEnabled) {
			chosenDelivery.setStatus(DeliveryStatus.Pending);
			EkrutClientUI.client_control.ekrut_client
			.handleMessageFromClientUI(new Command("update_delivery", chosenDelivery));
			delivery_details.setText("Delivery has been updated successfully");
			try {
				Thread.sleep(100);
			} catch (InterruptedException e) {
				e.printStackTrace();
			...
			message_to_gui.setText("You must choose Order in order to update");
		}
	}
}